...
public class ExcelFormatUtil {
    ...
    public static CellStyle headStyle(SXSSFWorkbook workbook){
        // 设置style的样式，此样式运用于第二行
        CellStyle style1 = workbook.createCellStyle();                      // cell样式
        // 设置单元格背景色，设置单元格背景色，以下两句必须同时设置
        style1.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);              // 设置填充样式
        style1.setFillForegroundColor(HSSFColor.GREY_25_PERCENT.index);     // 设置填充颜色
        // 设置单元格上下左右边框
    ...
    public static CellStyle contentStyle(SXSSFWorkbook wb){
        ...
        CellStyle style1 = wb.createCellStyle();                            // cell 样式
        // 设置单元格上、下、左、右的边框线
        style1.setBorderBottom(HSSFCellStyle.BORDER_THIN);
        style1.setBorderLeft(HSSFCellStyle.BORDER_THIN);
        style1.setBorderRight(HSSFCellStyle.BORDER_THIN);
        style1.setBorderTop(HSSFCellStyle.BORDER_THIN);
        style1.setWrapText(true);                                           // 设置自动换行
        style1.setAlignment(HSSFCellStyle.ALIGN_LEFT);                      // 设置单元格显示居左（左右方向）
        style1.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);         // 设置单元格字体显示居中（上下方向）
        return style1;
    }
     ...
     * @return
     */
    public static HSSFCellStyle titleStyle(HSSFWorkbook workbook, short color, short fontSize){
        // 设置style1的样式，此样式运用在第二行
